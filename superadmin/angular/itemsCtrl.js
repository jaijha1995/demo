app.controller('itemsCtrl', function($scope, $http, $window, $location, $sce, $timeout, store, config) {
    ...
    $scope.list = function(req, res) {
        $http.get(config.baseurl + 'loanform/')
    ...
    $scope.add = function(req, res) {
        ...
        if ($scope.data.id != "") {
            $http.put(config.baseurl + 'loanform/', $scope.data)
                .success(function(res) {
                    if (res.status == 'false') {} else {
        ...
        $http.post(config.baseurl + 'loanform/', $scope.data)
            .success(function(res) {
                if (res.status == 'false') {} else {
                    $scope.response = res.data;
                    console.log('message: ', $scope.response);
    ...
    $scope.update = function(id) {
        $http.get(config.baseurl + 'loanform/' + id)
            .success(function(res) {
                if (res.status == 'false') {} else {
                    $scope.data = res.data;
    ...
    $scope.delete = function(id) {
        $http.delete(config.baseurl + 'loanform/' + id)
            .success(function(res) {
                if (res.status == 'false') {} else {
                    $scope.response = res.data;
